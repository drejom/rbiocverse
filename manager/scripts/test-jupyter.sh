#!/bin/bash
# Test JupyterLab setup on HPC clusters
# Usage: ./test-jupyter.sh [gemini|apollo] [--gpu a100|v100]
...
if [ -n "$GPU" ]; then
  ...
  # Auto-select GPU queue based on idle nodes (mix state = has idle resources)
  if [ "$GPU" = "auto" ]; then
    echo "Checking GPU queue availability..."
    # Count nodes with idle resources (mix or idle state, not fully allocated)
    ...
    if [ "$V100_IDLE" -gt 0 ] && [ "$V100_IDLE" -ge "$A100_IDLE" ] 2>/dev/null; then
      ...
      # Both busy, pick V100 (usually shorter queue)
      GPU="v100"
...
echo "=== Testing JupyterLab on $CLUSTER ==="
echo "Host: $HOST"
echo "Image: $IMAGE"
if [ -n "$GPU" ]; then
...
# Test 2: Check JupyterLab is installed in container
echo ""
echo "2. Checking JupyterLab installation..."
ssh $HOST "$SINGULARITY exec $IMAGE jupyter --version 2>/dev/null | head -3"

# Test 3: Check Python path exists (if set up)
echo ""
echo "3. Checking Python site-packages..."
ssh $HOST "test -d $PYTHON_PATH && echo '   ✓ Python path exists' || echo '   ✗ Python path NOT found (may need setup)'"
...
# Test 5: Check kernels available
echo ""
echo "5. Checking Jupyter kernels..."
ssh $HOST "$SINGULARITY exec --env R_LIBS_SITE=$R_LIBS_SITE -B $BIND_PATHS $IMAGE jupyter kernelspec list 2>/dev/null"

# Test 6: Submit test job
echo ""
echo "6. Submitting test SLURM job..."

# Minimal resources for quick test
SBATCH_CMD="sbatch --parsable --job-name=$JOB_NAME --partition=$PARTITION $GPU_ARGS --cpus-per-task=1 --mem=4G --time=00:05:00 --wrap='$SINGULARITY exec $SINGULARITY_GPU --env TERM=xterm-256color --env R_LIBS_SITE=$R_LIBS_SITE --env PYTHONPATH=$PYTHON_PATH -B $BIND_PATHS $IMAGE jupyter lab --ip=0.0.0.0 --port=$PORT --no-browser --ServerApp.token=\"\" --ServerApp.password=\"\" --ServerApp.root_dir=\$HOME --ServerApp.base_url=/jupyter-direct'"
...
echo ""
echo "7. Waiting for job to start..."
for i in {1..30}; do
...
if [ "$STATE" = "RUNNING" ]; then
  echo ""
  echo "8. Waiting for JupyterLab to start (up to 30s)..."

  for i in {1..15}; do
    RESPONSE=$(ssh $HOST "curl -s -o /dev/null -w '%{http_code}' http://$NODE:$PORT/jupyter-direct/ 2>/dev/null || echo '000'")
    if [ "$RESPONSE" = "200" ] || [ "$RESPONSE" = "302" ]; then
      echo "   ✓ JupyterLab responding (HTTP $RESPONSE) after ${i}x2 seconds"
      break
    fi
    echo -n "."
    sleep 2
  ...
  if [ "$RESPONSE" != "200" ] && [ "$RESPONSE" != "302" ]; then
    echo "   ✗ JupyterLab not responding (got: $RESPONSE)"
  ...
  if [ -n "$GPU" ]; then
    ...
    if [ "$GPU_CHECK" != "FAILED" ] && [ -n "$GPU_CHECK" ]; then
      ...
      ssh $HOST "srun --jobid=$JOB_ID $SINGULARITY exec --nv $IMAGE nvidia-smi 2>&1 | head -10"
    fi
  fi

  echo ""
  echo "10. Cancelling test job..."
