...
class TunnelService {
   ...
   *
   * Uses lsof directly via execSync for reliable PID detection.
   * Requires lsof to be installed (included in Alpine Docker image).
   */
  async forceReleasePort(port: number): Promise<boolean> {
    try {
      // Use lsof -t to get PIDs for any process bound to the target port
