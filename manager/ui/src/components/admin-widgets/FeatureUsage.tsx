...
interface FeatureData {
  shiny?: { percent: number; count: number };
  liveServer?: { percent: number; count: number };
  vscodeTotal: number;
}
...
export function FeatureUsage({ getAuthHeader }: FeatureUsageProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      log.error('Failed to fetch feature usage', { error: err });
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    g.selectAll('.bar')
      ...
      .attr('height', y.bandwidth())
      .attr('fill', (_d, i) => i === 0 ? 'var(--status-success)' : 'var(--accent-primary)')
      .attr('rx', 3);
