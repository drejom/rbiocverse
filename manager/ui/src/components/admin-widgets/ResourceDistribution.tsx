...
export function ResourceDistribution({ getAuthHeader }: ResourceDistributionProps) {
  ...
  useEffect(() => {
    ...
    const x = d3.scaleBand()
      .domain(cpuData.map(d => String(d.cpus)))
      .range([0, width])
      .padding(0.1);

    const y = d3.scaleLinear()
      .domain([0, d3.max(cpuData, d => d.count) || 1])
      .nice()
      .range([height, 0]);
    ...
    g.append('g')
      ...
      .call(d3.axisBottom(x).tickValues(cpuData.filter((_d, i) => i % 2 === 0 || cpuData.length <= 8).map(d => String(d.cpus))))
      .selectAll('text')
      .attr('font-size', '9px')
  ...
  const formatMemory = (patterns: MemoryPattern[] | undefined) => {
