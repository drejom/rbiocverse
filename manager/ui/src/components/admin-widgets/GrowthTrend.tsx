...
export function GrowthTrend({ getAuthHeader }: GrowthTrendProps) {
  ...
  useEffect(() => {
    ...
    // Lines
    const sessionsLine = d3.line<ParsedGrowthData>()
      .x(d => x(d.date))
      .y(d => ySessions(d.sessions))
      .curve(d3.curveMonotoneX);

    const usersLine = d3.line<ParsedGrowthData>()
      .x(d => x(d.date))
    ...
    g.append('g')
      ...
      .call(d3.axisBottom(x).ticks(6).tickFormat(d3.timeFormat('%b') as (d: Date | d3.NumberValue, i: number) => string))
      .selectAll('text')
