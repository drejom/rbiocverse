...
export function AdoptionCurve({ getAuthHeader, version }: AdoptionCurveProps) {
  const svgRef = useRef<SVGSVGElement>(null);
  const [data, setData] = useState<AdoptionDataPoint[]>([]);
  ...
  useEffect(() => {
    const fetchVersions = async () => {
      try {
        ...
        const versionList = (json.data || []).map((d: { version: string }) => d.version).filter(Boolean);
  ...
  useEffect(() => {
    ...
    const y = d3.scaleLinear()
      .domain([0, d3.max(chartData, d => d.users) || 1])
      .nice()
      .range([height, 0]);

    // Area
    const area = d3.area<ParsedDataPoint>()
      .x(d => x(d.date))
      .y0(height)
      .y1(d => y(d.users))
